...
- Portkey fallback routing through a saved gateway configuration when the primary Groq virtual key fails.
...
Important backend values include Firebase Admin credentials, `JINA_API_KEY`, `PORTKEY_API_KEY`, the saved Portkey fallback slug (`PORTKEY_CONFIG_ID`), and the `SMTP_*` Gmail App Password settings. Configure the saved Portkey configuration with separate primary and fallback Groq virtual keys, then copy its `pc-...` slug into `backend/.env`. Never commit these values.
